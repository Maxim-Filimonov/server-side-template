...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Meal Planner Server account.<br>
        ...
        &mdash; Meal Planner Server Team
      `
      return sendMail({ toEmail: email, subject: 'Meal Planner Server - Password Reset', content })
    })
